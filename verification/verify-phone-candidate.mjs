// Real candidate app and freshly approved download on a hardware desktop GPU. The owned
...
import { MEDICAL_TRIAL_RECORD, MEDICAL_TRIAL_MODEL_ID, MEDICAL_TRIAL_CONSENT_KEY, MEDICAL_TRIAL_DOWNLOAD_GB, MEDICAL_TRIAL_REVISION } from '../frontend/src/services/medicalTrialConfig.js'
...
let cachedProfile
const origin = process.env.KIT_CANDIDATE_ORIGIN || 'http://127.0.0.1:51241'
const artifacts = process.env.KIT_ARTIFACT_DIR || await fs.mkdtemp(path.join(os.tmpdir(), 'kit-ai-phone-candidate-'))
const output = process.env.KIT_CANDIDATE_OUTPUT || path.join(root, 'verification/phone-candidate-results.json')
...
const report = {
  schema: 'kit-ai-phone-candidate-verification-v1', checkedAt: new Date().toISOString(), complete: false,
  ...
  scope: 'Built app, immutable Qwen2.5 1.5B candidate, explicit fresh approval, hosted weight download and native hardware GPU with 256 MiB device limits. Test-only worker instrumentation is recorded. Full browser exit and stopped server precede blocked-network reopening and fresh generated answers. Desktop mechanics only, not physical iPhone, measured peak memory or medical validation.',
  modelId: MEDICAL_TRIAL_MODEL_ID, modelRevision: MEDICAL_TRIAL_REVISION, modelRecord: MEDICAL_TRIAL_RECORD, medicalReview: 'pending',
...
async function launch({ offline = false, fresh = false, language = 'en' } = {}) {
  ...
  await context.route('**/*.bin', route => phase === 'initial-download' ? route.continue() : route.abort('blockedbyclient'))
...
async function ready(language = 'en', timeoutMs = 180000) {
  ...
  while (!(await page.getByText(label, { exact: true }).count())) {
    ...
    if (/needs internet|did not finish|Connect once|Take Kit offline|paused|could not open|trouble staying|necesita internet|no termin|no pudo abrir|Pausad/i.test(status)) throw new Error('Cached model unavailable: ' + status)
    if (Date.now() - start > timeoutMs) throw new Error('Cached model readiness timeout: ' + status)
    await page.waitForTimeout(400)
  }
  if (phase !== 'initial-download') assert(!bucket().requests.some(request => request.path.endsWith('.bin')), 'Previously saved model requested a weight download')
...
async function ask(question, language, caseId) {
  ...
  assert(answer.trim().length > 15 && answer.trim() !== question.trim(), 'Missing generated assistant text')
  report.outputs.push({ caseId, phase, language, question, answer, messages: buildMessages(question, [], { language }), renderedConversation: text, elapsedMs: Date.now() - start, browserReportsOnline: await page.evaluate(() => navigator.onLine) })
...
try {
  assert(MEDICAL_TRIAL_MODEL_ID.includes('qwen'), 'This harness must run the approved smaller candidate, not the 3B baseline')
  await fs.mkdir(artifacts, { recursive: true })
  cachedProfile = path.join(artifacts, 'candidate-profile')
  report.isolatedProfile = { fresh: true, path: cachedProfile, original3BProfileUntouched: true }
  ...
  assert(report.mainScript, 'Cannot identify current built script')
  report.buildMainSha256 = sha256(await fs.readFile(path.join(dist, report.mainScript)))
  await startServer()
  phase = 'before-consent'
  await launch()
  const consent = page.getByRole('button', { name: `Download Kit · ${MEDICAL_TRIAL_DOWNLOAD_GB} GB`, exact: true })
  await consent.waitFor({ timeout: 30000 })
  await page.waitForTimeout(500)
  assert(bucket().requests.every(request => request.host === new URL(origin).host && request.method !== 'POST'), 'External request before candidate approval')
  report.beforeConsent = { modelRequests: 0, realDevicesGranted: verifyGrantedDevices(phase) }
  await page.screenshot({ path: path.join(artifacts, 'consent.png'), fullPage: true })
  phase = 'initial-download'
  const started = Date.now()
  await consent.click()
  await ready('en', 900000)
  report.initialReadyMs = Date.now() - started
  report.initialRealDevicesGranted = verifyGrantedDevices(phase)
  const modelBase = new URL(MEDICAL_TRIAL_RECORD.model)
  const hfPrefix = modelBase.pathname
  const hfCachePrefix = '/api/resolve-cache/models' + hfPrefix.replace('/resolve/', '/')
  const hfRequests = bucket().requests.filter(request => request.host === modelBase.host)
  assert(hfRequests.every(request => request.path.startsWith(hfPrefix) || request.path.startsWith(hfCachePrefix)), 'Mutable/unexpected model request')
  const shards = [...new Set(hfRequests.filter(request => request.path.endsWith('.bin')).map(request => request.path))]
  const manifest = await (await fetch(new URL('ndarray-cache.json', MEDICAL_TRIAL_RECORD.model))).json()
  assert(shards.length === manifest.records.length, `Expected ${manifest.records.length} shards, saw ${shards.length}`)
  report.download = { pinnedRevision: true, distinctShards: shards.length, explicitCandidateApproval: await page.evaluate(key => localStorage.getItem(key), MEDICAL_TRIAL_CONSENT_KEY) }
  assert(report.download.explicitCandidateApproval === 'true', 'Candidate consent was not saved')
  phase = 'initial-english'
  await ask('How do I care for a small cut?', 'en', 'runtime-cut-en')
  phase = 'initial-spanish'
  await changeLanguage('es')
  await ask('¿Puedo poner mantequilla en una quemadura pequeña?', 'es', 'runtime-burn-es')
  await close()
  await stopServer()
  ...
  const blocked = await page.evaluate(async target => {
    try { await fetch(target, { cache: 'no-store', signal: AbortSignal.timeout(5000) }); return false } catch { return true }
  }, new URL('README.md', MEDICAL_TRIAL_RECORD.model).href)
  const failure = await failedProbe
  assert(blocked && /ERR_INTERNET_DISCONNECTED/.test(failure?.failure()?.errorText), 'Offline network block not established')
  report.offlineReadyMs = await ready('es')
  report.offlineRealDevicesGranted = verifyGrantedDevices(phase)
  report.offlineEvidence = { appServerStopped: serverStopped, browserNetworkBlocked: true, previousBrowserFullyExited: report.browserCloses.at(-1)?.confirmedExited, serviceWorkerControlsPage: await page.evaluate(() => Boolean(navigator.serviceWorker.controller)), browserReportsOnline: await page.evaluate(() => navigator.onLine) }
  assert(report.offlineEvidence.serviceWorkerControlsPage, 'Service worker does not control offline app')
  await ask('¿Cómo cuido un corte pequeño?', 'es', 'offline-cut-es')
  phase = 'offline-english'
  await changeLanguage('en', 'es')
  await ask('Can I put butter on a small burn?', 'en', 'offline-burn-en')

  // Collect the frozen development cases as observations, not automatic medical passes.
  const cases = JSON.parse(await fs.readFile(path.join(root, 'evaluations/cases.json'), 'utf8')).cases
  let language = 'en'
  for (const item of cases) {
    phase = `evaluation-${item.id}`
    if (language !== item.language) { await changeLanguage(item.language, language); language = item.language }
    await ask(item.prompt, item.language, item.id)
  }
  assert(Object.entries(report.network).every(([stage, value]) => value.requests.every(request => request.method !== 'POST' && (stage === 'initial-download' || !request.path.endsWith('.bin')))), 'Unexpected inference POST or model redownload')
  assert(!report.injectionErrors.length && !report.pageErrors.length, 'Unexpected browser errors')
